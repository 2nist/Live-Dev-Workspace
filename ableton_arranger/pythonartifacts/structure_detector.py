...
def test_structure_detection(audio_path: str, api_key: str = "") -> List[SectionData]:
    """
    Test function for structure detection.
    
    Args:
        audio_path: Path to test audio file
        api_key: SONOTELLER.AI API key (optional)
        
    Returns:
        Detected sections
    """
    config = AnalysisConfig()
    config.sonoteller_api_key = api_key
    
    detector = StructureDetector(config)
    sections = detector.detect_sections(audio_path)
    
    print(f"Detected {len(sections)} sections:")
    for i, section in enumerate(sections):
        print(f"  {i+1}. {section.name}: {section.start_time:.1f}s - {section.end_time:.1f}s "
...
if __name__ == "__main__":
    ...
    if os.path.exists(test_file):
        ...
        print("No test file found at /tmp/test_audio.mp3")
